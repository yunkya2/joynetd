...
接続先のハブやスイッチとの組み合わせによっては、リンクアップや auto-negotiation が安定しないことがあります。
そのような場合は `-v` オプションで起動時の PHY 状態を表示しつつ、`-m` オプションで PHY モードを切り替えて動作を確認してください。
...
`-m` を省略した場合は、従来通り W5500 のハードウェア PMODE 設定を使用します。
`-mauto` を指定すると、ソフトウェア側で auto-negotiation を有効にします。
`-m100f` や `-m10f` で接続が安定する場合は、接続先との auto-negotiation もしくは PHY モード設定に起因する問題の可能性があります。
いずれのモードでも `link=down` のまま変化しない場合は、LANケーブル、電源供給、モジュール実装、あるいは接続先ポート側の問題を疑ってください。
...
- `-m <phy mode>`: PHYモードを指定する
  - 起動時の PHY モードをソフトウェアから指定します。未指定時は W5500 のハードウェア PMODE 設定を使用します
  - 指定できる値は `auto`, `10h`, `10f`, `100h`, `100f`, `100h_an`, `pdown` です
  - 接続先によって auto-negotiation が不安定な場合の切り分けに使用できます
